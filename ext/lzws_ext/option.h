...
#define LZWS_EXT_GET_ULONG_OPTION(options, type, name) \
  type name = lzws_ext_get_ulong_option_value(options, #name);

#define LZWS_EXT_GET_COMPRESSOR_OPTIONS(options)                         \
  LZWS_EXT_GET_BOOL_OPTION(options, without_magic_header);               \
  LZWS_EXT_GET_ULONG_OPTION(options, uint_fast8_t, max_code_bit_length); \
...
#define LZWS_EXT_GET_BUFFER_LENGTH_OPTION(options, name) \
  LZWS_EXT_GET_ULONG_OPTION(options, size_t, name);
